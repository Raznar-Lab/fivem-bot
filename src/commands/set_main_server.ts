...
export const SetMainServer: ICommand = {
    ...
    execute: async (config, client, interaction) => {
        ...
        try {
            const data = await axios.get(`${config.data.main.host}/info.json`)
            if(data.data != null) {
        ...
        config.save();
        updateInterval(client, config.data.main)
